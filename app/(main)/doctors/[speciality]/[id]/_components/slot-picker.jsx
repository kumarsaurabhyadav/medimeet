  ...
  return (
    <div className="space-y-6">
      <Tabs
        ...
        className="w-full"
      >
        <TabsList className="w-full justify-start overflow-x-auto px-2 space-x-2">
          {days.map((day) => (
            <TabsTrigger
              key={day.date}
              value={day.date}
              disabled={day.slots.length === 0}
              className={`transition-all focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none ${
                    ...
                    return (
                      <Card
                        ...
                        className={`border-emerald-900/20 cursor-pointer transition-all ${
                          isSelected
                            ? "bg-emerald-800 border-emerald-500 shadow-lg"
                            : "hover:border-emerald-700/40 hover:bg-emerald-950/20"
                        }`}
                        onClick={() => handleSlotSelect(slot)}
                      >
                        <CardContent className="p-3 flex items-center" title={format(new Date(slot.startTime), "h:mm a")}>
                          <Clock
